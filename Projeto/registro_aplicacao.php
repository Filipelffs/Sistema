...
<body>
  ...
  <div class="card card-premium">
    ...
    <div class="card-body">
      <form id="aplicacaoForm">
        ...
        <div class="form-group-custom mb-3">
          <label>Dose</label>
          <select id="aplDose" class="form-select form-control-custom-noicon" required>
            ...
            <option value="3ª dose (quando necessário)">3ª dose (quando necessário)</option>
        ...
        <div class="form-group-custom mb-3">
          ...
          <select id="aplVia" class="form-select form-control-custom-noicon" required>
            <option value="" disabled selected>Selecione a via...</option>
            <option value="Intramuscular (IM)">Intramuscular (IM)</option>
            <option value="Subcutânea (SC)">Subcutânea (SC)</option>
            <option value="Intravenosa (IV)">Intravenosa (IV)</option>
            <option value="Oral">Oral</option>
            <option value="Intranasal">Intranasal</option>
            <option value="Tópica (sobre a pele)">Tópica (sobre a pele)</option>
          </select>
        </div>

        <!-- Volume administrado -->
        <div class="form-group-custom mb-3">
          <label>Volume administrado</label>
          <div class="d-flex gap-2 align-items-center">
            <input type="number" step="0.01" min="0" id="aplVolume" class="form-control-custom-noicon" placeholder="Ex: 5.0" style="flex:1;">
            <select id="aplVolumeUnidade" class="form-select form-control-custom-noicon" style="max-width:200px;">
  ...
  <script>
    document.getElementById("aplicacaoForm").addEventListener("submit", async function(e) {
      ...
      const dose     = document.getElementById("aplDose").value;
      const via      = document.getElementById("aplVia").value;
      const volume   = document.getElementById("aplVolume").value;
      const unidade  = document.getElementById("aplVolumeUnidade").value;
      const tipoStr  = dose + ' | Via: ' + via + (volume ? ' | Volume: ' + volume + ' ' + unidade : '');
